...
const renderCalendar = () => {
	...
	dates.forEach((date, i) => {
	  ...
	  dates[i] = "<div class ='date' onclick=\"choiceDay(this,\'"+(firstDateIndex+date-1)+"\',\'"+condition+"\')\"><span class =\'"+condition+"\'>"+date+"</span><div name='insertData'></div></div>";
...
function modalOpen() {
   ...
   modal.style.display = "flex";
	let team = document.getElementsByName("teamChoice")[0];
	let tCode = document.getElementsByName("tCode")[0].value.split(",");
	let tName = document.getElementsByName("tName")[0].value.split(",");
	let getDate = document.getElementsByName("getDate")[0].innerText;
	let dateDiv = document.getElementById("dateDiv");
	let html="";
	let dateDivs = "";
	
	html+="<select name=\"tecode\">";
	html+="<option>팀 선택</option>";
	for(i=0;i<tCode.length; i++){
		html+="<option value=\""+tCode[i]+"\">"+tName[i]+"</option>";}
	html+="</select>";
	
	dateDivs+="<input type=\"hidden\" name=\"dates\" value=\""+getDate+"\">";
	dateDivs+="<div>"+getDate+"</div>";
	
	dateDiv.innerHTML = dateDivs;
	team.innerHTML = html;
}

/*function openOne(a){
	let check = document.getElementsByName("open");
